...
import { ChevronDownIcon, ChevronRightIcon, Pencil } from "lucide-react";
...
export default function CategorySidebar(props: Props) {
  ...
  const handleAddTopic = (subcatId: number, subcatName: string) => {
    ...
    if (newTopic && newTopic.trim() !== "") {
      addTopic(subcatId, newTopic.trim());
    }
  }
  ...
  const handleAddSubcategory = (category_id: number, category: string) => {
    ...
    if (newSubcat && newSubcat.trim() !== "") {
      addSubcategory(category_id, newSubcat.trim());
    }
  }
  ...
  const handleEditTopic = (topicName: string, topicId: number) => {
    ...
    if (newName && newName.trim() !== "") {
      updateTopic(topicId, newName.trim());
    }
  }

  const handleEditSubcategory = (subcatId: number, subcatName: string) => {
    const newName = prompt(
      `Edit subcategory "${subcatName}":`,
      subcatName
    );
    if (newName && newName.trim() !== "") {
      updateSubcategory(subcatId, newName.trim());
    }
  }
        ...
        return (
          <div key={category} className="mb-4">
            <button
              ...
              className="flex items-center justify-between w-full text-left px-2 py-2 font-bold rounded hover:bg-gray-200"
            >
              <span>{category}</span>
              {isCategoryOpen ? (
                            ...
                            >
                              <div
                                className={`px-2 py-1 rounded cursor-pointer flex-1 ${selected?.category === category &&
                                  selected?.subcategory === topic.name &&
                                  selected?.topic === topic.name
                                  ? "bg-dvrpc-blue-1 text-white"
                                  : "hover:bg-gray-300"
                                  }`}
                                onClick={() =>
                                  handleItemClick(category, subcat.name, topic.name, topic.content_id)
                                }
                              >
                                {topic.name}
                              </div>
